...
def build_a2ui_autonomy_instruction(language: str = "en") -> str:
    ...
    template_binding_rule_en = (
        ...
        "updateDataModel keys are invalid. Inside template components, "
        "paths for Text, Image, and Button.action.event.context values MUST "
        "be relative, meaning they never start with a slash: 'name', "
        "'price', 'image' are correct; '/name', '/image', '/item/name', "
        "'/phones/name' are all wrong in exactly the same way - any path "
        "starting with '/' is resolved from the data model's absolute root, "
        "completely ignoring which item the template is currently "
        "rendering, so it silently reads the wrong (usually undefined) "
        "value for every item instead of that item's own field. "
    ...
    template_binding_rule_zh = (
        ...
        "空列表渲染出零个条目。updateDataModel 的 key 不能重复。模板组件内 "
        "Text、Image、Button.action.event.context 的 path 必须是相对路径，"
        "即绝不能以斜杠开头：name、price、image 是正确写法；/name、/image、"
        "/item/name、/phones/name 全都是同一种错误——任何以 / 开头的 path 都会"
        "从 data model 的绝对根路径解析，完全无视模板当前渲染到第几个 item，"
        "导致每个 item 都读到错误（通常是 undefined）的值，而不是那个 item "
        "自己的字段。A2UI 0.9.1 不要在模板渲染出的组件内部再嵌套 template；请把重复 item 的"
