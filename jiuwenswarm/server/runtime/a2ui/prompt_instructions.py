...
def build_a2ui_autonomy_instruction(language: str = "en") -> str:
    ...
    redundant_value_text_rule_en = (
        ...
        "TextField, CheckBox, DateTimeInput, MultipleChoice) that already "
    ...
    design_guideline_rule_en = (
        " Visual design guidance for a professional result: use Text usageHint "
        ...
        "short label directly above its MultipleChoice/control — never let "
    ...
    ui_ux_expertise_rule_en = (
        ...
        "one Text usageHint for the surface's own title (h1 or h2) and one "
        ...
        "interaction pattern to match the data, not habit: use chips for up "
        "to about 5 short mutually exclusive options, a checkbox list when "
        "options carry longer descriptive text, and a dropdown with "
        "filterable: true when there are more than about 8 options (long "
        "lists like countries or cities always need a dropdown, never a "
        "wall of chips). Write every label, heading, and button in the "
    ...
    requested_component_rule_en = (
        ...
        "or MultipleChoice — bind its value/selections property to a data "
        "model path, initialize that path with dataModelUpdate, and include "
        "the submitted value in Button.action.context using a path reference. "
        "A MultipleChoice or chips component with no bound selections path "
        "will not respond to clicks. Do not emit an empty "
        "Button.action.context for form submissions. Never use environment "
    ...
    design_guideline_rule_zh = (
        ...
        "必须为每个维度单独加一个简短的小标签放在对应 MultipleChoice/控件正上方——"
    ...
    ui_ux_expertise_rule_zh = (
        ...
        "usageHint（h1 或 h2），为分区小标题只选定一个级别（h3 或 h4），并在整个 "
        "surface 中始终使用这两个级别，不要跳级或随意混用不同标题大小。根据数据"
        "形态而不是习惯选择交互控件：约 5 个以内的简短互斥选项用 chips，选项带"
        "较长说明文字时用 checkbox 列表，超过约 8 个选项时用带 filterable: "
        "true 的下拉框（国家、城市这类长列表必须用下拉框，绝不能铺一整墙 "
        "chips）。所有 label、标题、按钮都要用用户描述自己目标时会说的话，绝"
    ...
    requested_component_rule_zh = (
        ...
        "Slider、CheckBox、DateTimeInput 或 MultipleChoice——都必须把其 value/"
        "selections 属性绑定到 data model 路径，用 dataModelUpdate 初始化该路径，"
        "并在 Button.action.context 中用 path reference 包含提交值。没有绑定"
        "selections 路径的 MultipleChoice 或 chips 组件点击后不会有任何反应。"
        "表单提交不能输出空的 Button.action.context。"
